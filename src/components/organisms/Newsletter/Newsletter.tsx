...
export function Newsletter() {
  ...
  return (
    <form action='/subscribers' onSubmit={onSubmitForm} method='POST' className='rounded-xl border border-default p-6 dark:border-zinc-700/40'>
      ...
      <p className='mt-2 text-sm text-zinc-600 dark:text-zinc-400'>{t('content')}</p>
      <div className='mt-6 flex'>
        <input type='email' name='email' className='h-10 min-w-0 flex-auto appearance-none rounded-lg border border-zinc-900/10 bg-white px-3 py-[calc(theme(spacing.2)-1px)] shadow-md shadow-zinc-800/5 placeholder:text-zinc-400 focus:border-teal-500 focus:outline-none focus:ring-4 focus:ring-teal-500/10 text-sm dark:border-zinc-700 dark:bg-zinc-700/[0.15] dark:text-zinc-200 dark:placeholder:text-zinc-500 dark:focus:border-teal-400 dark:focus:ring-teal-400/10' placeholder={t('email placeholder')} aria-label='Email address' required />
